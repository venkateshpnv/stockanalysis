...
import st_demo3_functions as functions
...
start_date = st.sidebar.date_input('Start date:',
                                   today - timedelta(days=365*1),  # The default time frame is 1 year.
                                   min_value=today - timedelta(days=365*4),
                                   max_value=today - timedelta(days=31*2))
...
if ticker:
    ...
    if df.shape[0] == 0:
        ...
        fig.update_xaxes(rangebreaks=[dict(values=closed_dates_list)],
                         range=[dt.strptime(df['Date'][0], "%Y-%m-%d").date() - timedelta(days=3), dt.strptime(df['Date'][-1], "%Y-%m-%d").date() + timedelta(days=3)])
