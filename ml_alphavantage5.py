...
# === MAIN RUN ===
def process_symbols(symbols, cpu_affinity=None):
    """Processes a list of stock symbols with optional CPU affinity."""
    if cpu_affinity is not None:
        try:
            os.sched_setaffinity(0, {cpu_affinity})  # Set CPU affinity for the current process
...
if __name__ == "__main__":
    ...
    end_date = dt.now().date()
    processes = []
    num_processes = 24 # You can adjust this based on your system's core count
    cpu_list = list(range(num_processes))  # Create a list of CPU cores to use

    # Read the API key from the file
    api_key_file = os.path.expanduser('/home/vpetla/alphavantage_token_file.txt')
    try:
        with open(api_key_file, 'r') as f:
            API_KEY = f.read().strip()  # Read and remove leading/trailing whitespace
    except FileNotFoundError:
        print(f"Error: API key file not found at {api_key_file}")
        exit(1)  # Exit if the file is not found

    for i in range(0, len(SELECTED_STOCKS), 10):
        stock_batch = SELECTED_STOCKS[i:i + 10]
        # Use a round-robin approach to assign CPU affinity
        cpu_affinity = cpu_list[i % len(cpu_list)]
        process = multiprocessing.Process(target=process_symbols, args=(stock_batch, cpu_affinity))
